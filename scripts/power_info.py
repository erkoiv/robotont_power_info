...
class PowerInfo:
    def __init__(self):
        ...
        self.powerData = {
            ...
            "draw": 0.0,
            "startingCapacity": 0.0,
            "batteryPercentageV": 0.0,
            "batteryPercentageA": 0.0,
            "remainingCapacity": 0.0
        }

        self.startingTime = self.powerData[
            "time"
        ]  # Set node starting time used in shutdown flagging
        self.df = pd.DataFrame(
            self.powerData, index=[0]
        )  # Initialize "empty row", first row can be sent in callback under first run conditional instead if node starting time not necessary
    ...
    def callback(self, data):
        if (len(self.Varray) < 9 or len(self.Aarray) < 9):  # For 9 measurements only listen
            ...
            if self.batMax - V >= self.batRange:
                self.powerData["batteryPercentageV"] = 0.0
            else:
                self.powerData["batteryPercentageV"] = round(
            ...
            if self.firstRun:
                self.powerData["startingCapacity"] = self.powerData["maxCapacity"] * (
                    self.powerData["batteryPercentageV"] / 100
                )
            ...
            # Calculate battery percentage from A data
            self.powerData["batteryPercentageA"] = self.powerData["remainingCapacity"]/self.powerData["maxCapacity"]
    ...
    def shutDown(self):
        ...
        self.df.to_csv(
            self.path + "/data/" + id + ".csv"
        )  # Save the measurements as csv
